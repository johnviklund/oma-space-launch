...
function cache(next, expiry = expiresAt) {
    return { schemaVersion: 1, expiresAt: expiry, next };
}

test("parseCache accepts only LaunchCacheV1 JSON", () => {
    assert.deepEqual(model.parseCache('{"schemaVersion":1,"next":null}'), { schemaVersion: 1, next: null });
